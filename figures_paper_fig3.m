...
set(gca,'YDir','normal');xlabel('time');ylabel('frequency');xlim([0 1]); set(gca,'XTick',0:0.2:1)
...
srec1 = srec(2,:);  q1 = squeeze(q5(1,:,:));
srec2 = srec(3,:);  q2 = squeeze(q5(2,:,:));
srec3 = srec(1,:);  q3 = squeeze(q5(3,:,:));
...
%set(FigHandle(7),'units','normalized','outerposition',[0 0 1 1]);
subplot(2,1,1)
plot(t,srec); xlim([0 1]);ylim([-5 15]);
legend('reconstruction'); %xlabel('time');
set(gca,'xtick',[]);
subplot(2,1,2)
plot(t,s0','r'); 
legend('noise-free'); xlabel('time');xlim([0 1]);ylim([-5 15]);% set(gca,'xtick',[]);
...
for i = 1:5
 export_fig(FigHandle(i), ... % figure handle
     sprintf('%s/3modes_%d', chemin0,i),... % name of output file without extension
     '-painters', ...      % renderer
