function [tfr,rtfr,hat] = tfrrsp_new(x,t,N,h,trace);
...
%size(tf2)
%figure();imagesc((real(tf2./tfr/Dt)))
%figure();imagesc(imag(N*tf3./tfr/(2.0*pi)))
%dx= real(tf2./tfr/Dt) + 1i.*(imag(N*tf3./tfr/(2.0*pi)));
%dx= round(real(tf2./tfr/Dt)) + 1i.*(round(imag(N*tf3./tfr/(2.0*pi))));
%ang = mod(angle(dx),pi);
%figure()
%histogram(ang)
%set(gca,'YDir','normal'); title('histogram of dx')
